...
A = [zeros(Ban,1,4) ones(Ban,1,2);
	 I M.*[-ones(Ban,2,1) zeros(Ban,2,1); zeros(Ban,2,1) -ones(Ban,2,1)]];

A_dom = [-1  0 zeros(Ban, 1, 2) 2  0;
		  0  1 zeros(Ban, 1, 2) 4  0;
		  zeros(Ban, 1, 2) -1 1 0 -1];
...
sol = ipqp(A,b,c,Q, tol; maxit=15, verbose=verbose, genLatex=genLatex, slack_var=7:size(A,2), bounded_variables=5:size(A,2));# slack_var=11:21);
