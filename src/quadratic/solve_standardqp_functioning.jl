function solve_standardqp(A,b,c,Q, tol=1e-8, maxit=100; verbose=false, genLatex=false, adj=true, slack_var=[])
    ...
    for iter=1:maxit
		...
		#x_aff -= retrieve_infinitesimals(x_aff, -1)
		#x_aff -= retrieve_infinitesimals(x_aff, max(degree(μ)-1, trash_deg))
		...
		#s_aff[findall(x->x.p<=trash_deg-2, s_aff)] .= 0
		λ_aff[findall(x->x.p<=trash_deg, λ_aff)] .= 0
		#λ_aff[findall(x->x.p<=trash_deg-2, λ_aff)] .= 0
		...
		#print("x_cc_true: "); println(x_cc)
		#println("")
		#print("n_s_cc_true: "); println(norm(s_cc))
		#print("s_cc_true: "); println(s_cc)
		#println("")
		...
		#x_cc -= retrieve_infinitesimals(x_cc, -1)
		#x_cc -= retrieve_infinitesimals(x_cc, max(degree(μ)-1, trash_deg))
		#s_cc -= retrieve_infinitesimals(s_cc, trash_deg)
		#λ_cc -= retrieve_infinitesimals(λ_cc, trash_deg)
		x_cc[findall(x->x.p<=trash_deg, x_cc)] .= 0
		s_cc[findall(x->x.p<=trash_deg, s_cc)] .= 0
		#s_cc[findall(x->x.p<=trash_deg-2, s_cc)] .= 0
		λ_cc[findall(x->x.p<=trash_deg, λ_cc)] .= 0
		#λ_cc[findall(x->x.p<=trash_deg-2, λ_cc)] .= 0
