...
include("solver3_u.jl")
include("alpha_max_u.jl")
include("starting_u.jl")
include("solve_standardqp_u.jl")
...
function ipqp(A::Matrix,b::Vector,c::Vector,Q::Matrix,tol; maxit=100, verbose=false, genLatex=false, slack_var=[], bounded_variables=[])
	...
	# Reorder variables as unbounded and then bounded
	unbounded_variables = setdiff(1:n0, bounded_variables)
	c = c[[unbounded_variables; bounded_variables]]
	A = A[:, [unbounded_variables; bounded_variables]]
	Q = Q[[unbounded_variables; bounded_variables], :]
	Q = Q[:, [unbounded_variables; bounded_variables]]
	
    ##############################
    # solve the original problem #
	##############################
    x,λ,s,flag,iter,r = solve_standardqp(A,b,c,Q,tol,maxit,verbose=verbose,genLatex=genLatex,slack_var=slack_var,bounded_variables=(length(unbounded_variables)+1):n0)
